--[[
    🍌 Banana Cat Hub — FUSION EDITION (A + B)
    - Giữ TOÀN BỘ tính năng bản gốc: quick script, lưu code, feature tab,
      resize 4 góc, drag, checkered BG, editor sửa, copy, search...
    - THÊM Sandbox Layer: hook ScreenGui.Parent, BindToRenderStep, task.spawn,
      cleanup khi đóng tab, watchdog chống destroy hub.
    - Không mất bất kỳ tính năng nào của bản A.
]]
...
-- ============================================================
-- SANDBOX LAYER (từ bản B — giữ nguyên)
-- ============================================================
local Sandbox = {
    activeHost   = nil,
    guisCreated  = {},
...
local function SandboxReset()
    Sandbox.activeHost = nil
    table.clear(Sandbox.guisCreated)
    table.clear(Sandbox.connsCreated)
    table.clear(Sandbox.renderBinds)
    table.clear(Sandbox.threads)
end

local function InstallScreenGuiHook()
    if Sandbox.hooksActive then return end
    Sandbox.hooksActive = true
    ...
    mt.__namecall = newcclosure(function(self, ...)
        ...
        if method == "new" and self == Instance and (...) == "ScreenGui" then
            local gui = oldNamecall(self, ...)
            table.insert(Sandbox.guisCreated, gui)
            return gui
        end

        if method == "__newindex" and self and typeof(self) == "Instance"
           and self.ClassName == "ScreenGui"
           and select(1, ...) == "Parent" then
            if Sandbox.activeHost then
                return oldNamecall(self, "Parent", Sandbox.activeHost)
            end
        end
...
local function InstallRunServiceHook()
    if Sandbox.hooksActive then return end
    local oldBind = RunService.BindToRenderStep
    RunService.BindToRenderStep = newcclosure(function(name, priority, fn)
        table.insert(Sandbox.renderBinds, name)
        return oldBind(name, priority, fn)
    end)
end

local function InstallThreadHook()
    if Sandbox.hooksActive then return end
    local oldSpawn = task.spawn
    task.spawn = newcclosure(function(fn, ...)
        local thread = oldSpawn(fn, ...)
        table.insert(Sandbox.threads, thread)
        return thread
    end)
end

local function SandboxCleanup()
    for _, c in ipairs(Sandbox.connsCreated) do
        pcall(function() c:Disconnect() end)
    end
    for _, name in ipairs(Sandbox.renderBinds) do
        pcall(function() RunService:UnbindFromRenderStep(name) end)
    end
    for _, t in ipairs(Sandbox.threads) do
        pcall(function() task.cancel(t) end)
    end
    for _, g in ipairs(Sandbox.guisCreated) do
...
local gui = New("ScreenGui", {
    Name="ExMenu", IgnoreGuiInset=true, ResetOnSpawn=false,
    ZIndexBehavior=Enum.ZIndexBehavior.Sibling,
}, targetGui)

-- ============================================================
-- TOGGLE BUTTON (giữ nguyên A)
-- ============================================================
local togBtn = New("TextButton", {
    Size=UDim2.new(0,48,0,48), Position=UDim2.new(1,-60,1,-60),
    Text="🍌", BackgroundColor3=C.WHITE, BackgroundTransparency=0.1,
    TextColor3=C.DARK, Font=Enum.Font.GothamBold, TextSize=24,
    BorderSizePixel=0, ZIndex=1000,
}, gui)
Corner(togBtn, UDim.new(1,0)); Stroke(togBtn, C.BLUE, 2)

-- ============================================================
-- MAIN FRAME (giữ nguyên A)
-- ============================================================
local main = New("Frame", {
    Size=UDim2.new(0,540,0,340), Position=UDim2.new(0.5,-270,0.5,-170),
    BackgroundColor3=Color3.fromRGB(235, 238, 245), BackgroundTransparency=0.25,
    BorderSizePixel=0, Visible=false, ClipsDescendants=false, ZIndex=2,
}, gui)
Corner(main, UDim.new(0,10)); Stroke(main, Color3.fromRGB(180,185,200), 2)

-- ============================================================
-- WATCHDOG (từ B — chèn sau khi main tồn tại)
-- ============================================================
trackConn(gui.Destroying:Connect(function()
    warn("[BananaCatHub] Script con cố destroy hub → phục hồi.")
    gui.Parent = targetGui
end))

local bgPattern = New("ImageLabel", {
    Name="CheckeredBG", Size=UDim2.new(1,0,1,0), Position=UDim2.new(0,0,0,0),
    BackgroundTransparency=1, Image="rbxassetid://9822602710",
    ScaleType=Enum.ScaleType.Tile, TileSize=UDim2.new(0,20,0,20),
    ImageTransparency=0.82, ImageColor3=Color3.fromRGB(150,160,185), ZIndex=2,
}, main)
Corner(bgPattern, UDim.new(0,10))

local titleBar = New("Frame", {
    Size=UDim2.new(1,0,0,30), BackgroundColor3=Color3.fromRGB(225,230,240),
    BackgroundTransparency=0.2, BorderSizePixel=0, ZIndex=3,
}, main)
Corner(titleBar, UDim.new(0,10))

New("TextLabel", {
    Size=UDim2.new(1,-90,1,0), Position=UDim2.new(0,12,0,0),
    Text="🍌 Banana Cat Executor Hub (Caro Style)", BackgroundTransparency=1,
    TextColor3=C.DARK, Font=Enum.Font.GothamBold, TextSize=13,
    TextXAlignment=Enum.TextXAlignment.Left, ZIndex=4,
}, titleBar)

local dragLockBtn = New("TextButton", {
    Size=UDim2.new(0,30,0,30), Position=UDim2.new(1,-64,0,0),
    Text="🔒", BackgroundTransparency=1, TextColor3=Color3.fromRGB(120,120,130),
    Font=Enum.Font.GothamBold, TextSize=15, BorderSizePixel=0, ZIndex=4,
}, titleBar)

local closeBtn = New("TextButton", {
    Size=UDim2.new(0,30,0,30), Position=UDim2.new(1,-32,0,0),
    Text="✕", BackgroundTransparency=1, TextColor3=Color3.fromRGB(120,120,130),
    Font=Enum.Font.GothamBold, TextSize=15, BorderSizePixel=0, ZIndex=4,
}, titleBar)

-- ============================================================
-- RESIZE HANDLES (giữ nguyên A)
-- ============================================================
...
local function SetupResizeHandle(btn, cornerType)
    ...
    trackConn(btn.InputBegan:Connect(function(i)
        if i.UserInputType==Enum.UserInputType.MouseButton1 or i.UserInputType==Enum.UserInputType.Touch then
            resizing=true; sizeStart=main.Size; posStart=main.Position; inputStart=i.Position
        end
    end))
    trackConn(UserInputService.InputChanged:Connect(function(i)
        if resizing and sizeStart and posStart and inputStart and (i.UserInputType==Enum.UserInputType.MouseMovement or i.UserInputType==Enum.UserInputType.Touch) then
            ...
            local newW, newH, newX, newY = w, h, posX, posY
            if cornerType == "BR" then
                newW = math.max(minW, w + d.X); newH = math.max(minH, h + d.Y)
            elseif cornerType == "BL" then
                newW = math.max(minW, w - d.X); newH = math.max(minH, h + d.Y); newX = posX + (w - newW)
            elseif cornerType == "TR" then
                newW = math.max(minW, w + d.X); newH = math.max(minH, h - d.Y); newY = posY + (h - newH)
            elseif cornerType == "TL" then
                newW = math.max(minW, w - d.X); newH = math.max(minH, h - d.Y)
                newX = posX + (w - newW); newY = posY + (h - newH)
            end
...
local function CreateHandle(icon, pos)
    local btn = New("TextButton", {
        Size=UDim2.new(0,20,0,20), Position=pos, Text=icon,
        BackgroundColor3=C.BLUE, BackgroundTransparency=0.1, TextColor3=C.WHITE,
        Font=Enum.Font.GothamBold, TextSize=11, BorderSizePixel=0, ZIndex=100,
    }, main)
    Corner(btn, UDim.new(0,5)); Stroke(btn, Color3.fromRGB(255,255,255), 1.2)
    return btn
end
...
-- ============================================================
-- TAB SYSTEM (giữ nguyên A)
-- ============================================================
local tabs, tabContent = {}, {}

local tabBar = New("ScrollingFrame", {
    Size=UDim2.new(0,105,1,-30), Position=UDim2.new(1,-105,0,30),
    BackgroundColor3=Color3.fromRGB(225,230,240), BackgroundTransparency=0.3,
    BorderSizePixel=0, ZIndex=3, ScrollBarThickness=3, CanvasSize=UDim2.new(0,0,0,0),
}, main)
...
local contentArea = New("Frame", {
    Size=UDim2.new(1,-105,1,-30), Position=UDim2.new(0,0,0,30),
    BackgroundTransparency=1, BorderSizePixel=0, ZIndex=3, ClipsDescendants=true,
}, main)
...
local function SwitchTab(index)
    ...
    for _, b in ipairs(tabs) do b.BackgroundColor3 = C.BG; b.BackgroundTransparency = 0.3 end
...
local function AddTab(name, icon, order, customContent)
    local btn = New("TextButton", {
        Size=UDim2.new(1,-8,0,30), Text=icon.." "..name,
        BackgroundColor3=C.BG, BackgroundTransparency=0.3, TextColor3=C.DARK,
        Font=Enum.Font.GothamBold, TextSize=9, BorderSizePixel=0,
        LayoutOrder=order, TextXAlignment=Enum.TextXAlignment.Left, ZIndex=4,
    }, tabBar)
    Corner(btn, UDim.new(0,6))

    local sf
    if customContent then
        sf = customContent; sf.Parent = contentArea; sf.Visible = false
    else
        sf = New("ScrollingFrame", {
            Size=UDim2.new(1,0,1,0), BackgroundTransparency=1, BorderSizePixel=0,
            ScrollBarThickness=5, ScrollBarImageColor3=Color3.fromRGB(120,120,140),
            ClipsDescendants=true, CanvasSize=UDim2.new(0,0,0,0), Visible=false,
            Active=true, Selectable=false, ScrollingDirection=Enum.ScrollingDirection.Y, ZIndex=4,
        }, contentArea)
    end

    local tabIdx = #tabs + 1
    btn.Activated:Connect(function() SwitchTab(tabIdx) end)
    table.insert(tabs, btn); table.insert(tabContent, sf)
...
local S = { dragMenu=false }

-- ============================================================
-- EXEC ENGINE (giữ nguyên A)
-- ============================================================
local scripts = {}
...
local function RunCode(code, name, ind, times, delay)
    ...
    curThread=task.spawn(function()
        for i=1,times do
            ...
            if ok then okC+=1 else warn("❌ Lần",i,err) end
        end
        totalRuns+=okC
...
-- ==================== TAB 1: CODE (giữ nguyên A) ====================
local y = 8
Label(codeTab, "⚡ Code Nhanh - Nhấn để chạy ngay", y); y += 16
...
y += 6
Label(codeTab, "━━━━━━━━━━━━━━━━━━━━━━", y); y += 16
Label(codeTab, "💻 Nhập Code Tùy Chỉnh", y); y += 14
Label(codeTab, "👤 Tên Script", y); y += 14
...
local nameIn = New("TextBox", {
    ...
    BackgroundColor3=C.WHITE, TextColor3=Color3.fromRGB(20,20,20),
...
Corner(nameIn, UDim.new(0,5)); Stroke(nameIn, Color3.fromRGB(100,120,200), 1.5)
New("UIPadding", {PaddingLeft=UDim.new(0,6)}, nameIn); y += 32

Label(codeTab, "💻 Code (Lua)", y); y += 14
...
local codeIn = New("TextBox", {
    ...
    BackgroundColor3=Color3.fromRGB(245,245,255), TextColor3=Color3.fromRGB(20,20,20),
    Font=Enum.Font.Code, TextSize=11, BorderSizePixel=0, ClearTextOnFocus=false,
    MultiLine=true, TextWrapped=true, TextXAlignment=Enum.TextXAlignment.Left,
    TextYAlignment=Enum.TextYAlignment.Top, Active=true, Selectable=true, ZIndex=10,
}, codeTab)
Corner(codeIn, UDim.new(0,5)); Stroke(codeIn, Color3.fromRGB(100,120,200), 1.5)
New("UIPadding", {PaddingLeft=UDim.new(0,6), PaddingTop=UDim.new(0,4)}, codeIn)
y += 86

Label(codeTab, "🔁 Cài đặt lặp", y); y += 14
...
Corner(repIn, UDim.new(0,5)); Stroke(repIn, Color3.fromRGB(180,180,200), 1.2)
...
Corner(delIn, UDim.new(0,5)); Stroke(delIn, Color3.fromRGB(180,180,200), 1.2)

local unitBtn = New("TextButton", {
    Size=UDim2.new(0,55,0,24), Position=UDim2.new(0,75,0,y+50), Text="Giây ▾",
    BackgroundColor3=C.WHITE, TextColor3=C.DARK, Font=Enum.Font.GothamBold,
    TextSize=9, BorderSizePixel=0, ZIndex=10,
}, codeTab)
Corner(unitBtn,UDim.new(0,4)); Stroke(unitBtn)

local ddFrame = New("Frame", {
    Size=UDim2.new(0,55,0,48), Position=UDim2.new(0,75,0,y+74),
    BackgroundColor3=C.WHITE, BorderSizePixel=0, Visible=false, ZIndex=15,
}, codeTab)
...
local secOpt = New("TextButton", {
    ...
    TextColor3=C.DARK, Font=Enum.Font.GothamBold, TextSize=9, BorderSizePixel=0, ZIndex=16,
}, ddFrame)

local minOpt = New("TextButton", {
    Size=UDim2.new(1,0,0,24), Position=UDim2.new(0,0,0,24), Text="Phút",
    BackgroundColor3=Color3.fromRGB(245,245,245), TextColor3=C.DARK,
...
runBtn.Activated:Connect(function()
    ...
    if not ok then statusLbl.Text=err or "❌ Lỗi không xác định"
    else
...
saveBtn.Activated:Connect(function()
    ...
    while true do
        local ex=false        for _,s in ipairs(scripts) do if s.name==n then ex=true; break end end
...
-- ==================== TAB 2: CODE ĐÃ LƯU (giữ nguyên A) ====================
local sy = 8
Label(savedCodeTab, "💾 Danh Sách Script Đã Lưu", sy); sy += 18
...
Corner(searchIn, UDim.new(0,5)); Stroke(searchIn, Color3.fromRGB(180,180,200), 1.2)
...
RebuildScripts = function()
    ...
    if #disp==0 then
        New("TextLabel", {
            ...
            BackgroundTransparency=1, TextColor3=C.GRAY, Font=Enum.Font.GothamMedium,
    ...
    for _, d in ipairs(disp) do
        ...
        local row = New("Frame", {
            Size=UDim2.new(1,0,0,rowH), BackgroundColor3=C.WHITE,
            BackgroundTransparency=0.1, BorderSizePixel=0, ZIndex=6, ClipsDescendants=true,
        }, scriptList)
        ...
        local arrowBtn = New("TextButton", {
            ...
            BackgroundColor3=Color3.fromRGB(220,225,240), TextColor3=C.BLUE,
        ...
        local delScriptBtn = New("TextButton", {
            ...
            Text="🗑 Xóa", BackgroundColor3=C.RED, TextColor3=C.WHITE,
            Font=Enum.Font.GothamBold, TextSize=10, BorderSizePixel=0, ZIndex=8,
        }, row)
        ...
        local runScriptBtn = New("TextButton", {
            ...
            Text="▶ Chạy", BackgroundColor3=C.GREEN, TextColor3=C.WHITE,
            Font=Enum.Font.GothamBold, TextSize=10, BorderSizePixel=0, ZIndex=8,
        }, row)
        ...
        if isExpanded then
            local codeBoxFrame = New("ScrollingFrame", {
                ...
                BackgroundColor3=Color3.fromRGB(240,242,250), BorderSizePixel=0,
                ZIndex=8, ScrollBarThickness=4, CanvasSize=UDim2.new(0,0,0,0),
            }, row)
            Corner(codeBoxFrame, UDim.new(0,5)); Stroke(codeBoxFrame, Color3.fromRGB(190,195,210), 1)
            ...
            local copyBtn = New("TextButton", {
                ...
                Text="📋 Sao Chép Code", BackgroundColor3=C.BLUE, TextColor3=C.WHITE,
                Font=Enum.Font.GothamBold, TextSize=10, BorderSizePixel=0, ZIndex=8,
            }, row)
            Corner(copyBtn, UDim.new(0,5))

            copyBtn.Activated:Connect(function()
                if setclipboard then pcall(setclipboard, d.code); copyBtn.Text = "✅ Đã Sao Chép!"
                elseif toclipboard then pcall(toclipboard, d.code); copyBtn.Text = "✅ Đã Sao Chép!"
                else
                ...
                task.delay(1.5, function()
                    if copyBtn and copyBtn.Parent then copyBtn.Text = "📋 Sao Chép Code" end
                end)
            end)
        end

        arrowBtn.Activated:Connect(function() d.expanded = not d.expanded; RebuildScripts() end)
        ...
        delScriptBtn.Activated:Connect(function()
            for idx, s in ipairs(scripts) do
                if s == d then table.remove(scripts, idx); break end
            end
            RebuildScripts()
        end)
...
local function ScanNewGuis(beforeGuis)
    ...
    local function scan(container)
        ...
        for _, g in ipairs(container:GetChildren()) do
            if not beforeGuis[g] then beforeGuis[g] = true; table.insert(found, g) end
        end
    end
    scan(playerGui); scan(targetGui)
    pcall(function()
...
-- RunFeatureScript: kết hợp ScanNewGuis (A) + Sandbox.activeHost (B)
    ...
    local ok, err = pcall(function()
        ...
        for _, g in ipairs(newGuis) do
            ...
            elseif g:IsA("GuiObject") then
                pcall(function() g.Parent = containerFrame; g.ZIndex = 5 end)
            end
        end
    end)
...
local function CreateFeatureTab(name, icon, codeContent)
    ...
    local featureData = {
        name=name, icon=icon, code=codeContent, btn=btn, frame=sf, tabIdx=tabIdx,
    }
    table.insert(featureTabs, featureData)

    local embedHost = New("Frame", {
        Size=UDim2.new(1,0,1,-36), Position=UDim2.new(0,0,0,0),
        BackgroundTransparency=1, BorderSizePixel=0, ZIndex=5,
        Name="ScriptHost", Visible=true,
    }, sf)

    local toolbar = New("Frame", {
        Size=UDim2.new(1,0,0,36), Position=UDim2.new(0,0,1,-36),
        BackgroundColor3=Color3.fromRGB(230,233,242), BackgroundTransparency=0.1,
        BorderSizePixel=0, ZIndex=20,
    }, sf)
    Corner(toolbar, UDim.new(0,6)); Stroke(toolbar, Color3.fromRGB(180,185,200), 1)

    local runFeatureBtn = New("TextButton", {
        Size=UDim2.new(0,110,0,26), Position=UDim2.new(0,6,0,5),
        Text="▶ Chạy Script", BackgroundColor3=C.GREEN, TextColor3=C.WHITE,
        Font=Enum.Font.GothamBold, TextSize=10, BorderSizePixel=0, ZIndex=21,
    }, toolbar); Corner(runFeatureBtn, UDim.new(0,5))

    local saveFeatureBtn = New("TextButton", {
        Size=UDim2.new(0,110,0,26), Position=UDim2.new(0,122,0,5),
        Text="💾 Lưu Vào DS", BackgroundColor3=C.BLUE, TextColor3=C.WHITE,
        Font=Enum.Font.GothamBold, TextSize=10, BorderSizePixel=0, ZIndex=21,
    }, toolbar); Corner(saveFeatureBtn, UDim.new(0,5))

    local editFeatureBtn = New("TextButton", {
        Size=UDim2.new(0,80,0,26), Position=UDim2.new(0,238,0,5),
        Text="✏️ Sửa", BackgroundColor3=C.ORANGE, TextColor3=C.WHITE,
        Font=Enum.Font.GothamBold, TextSize=10, BorderSizePixel=0, ZIndex=21,
    }, toolbar); Corner(editFeatureBtn, UDim.new(0,5))

    local closeFeatureBtn = New("TextButton", {
        Size=UDim2.new(0,40,0,26), Position=UDim2.new(1,-46,0,5),
        Text="✕", BackgroundColor3=C.RED, TextColor3=C.WHITE,
        Font=Enum.Font.GothamBold, TextSize=12, BorderSizePixel=0, ZIndex=21,
    }, toolbar); Corner(closeFeatureBtn, UDim.new(0,5))
    ...
    local editorFrame = New("Frame", {
        Size=UDim2.new(1,0,1,-36), Position=UDim2.new(0,0,0,0),
        BackgroundColor3=Color3.fromRGB(245,247,252), BorderSizePixel=0,
    ...
    local editorBox = New("TextBox", {
        ...
        Font=Enum.Font.Code, TextSize=11, BorderSizePixel=0, ClearTextOnFocus=false,
        MultiLine=true, TextWrapped=true, TextXAlignment=Enum.TextXAlignment.Left,
        TextYAlignment=Enum.TextYAlignment.Top, Active=true, Selectable=true, ZIndex=31,
    }, editorFrame)
    Corner(editorBox, UDim.new(0,5)); Stroke(editorBox, Color3.fromRGB(100,120,200), 1.5)
    ...
    local applyEditBtn = New("TextButton", {
        ...
        Text="✅ Áp Dụng", BackgroundColor3=C.GREEN, TextColor3=C.WHITE,
        Font=Enum.Font.GothamBold, TextSize=10, BorderSizePixel=0, ZIndex=31,
    }, editorFrame); Corner(applyEditBtn, UDim.new(0,5))

    local cancelEditBtn = New("TextButton", {
        Size=UDim2.new(0,120,0,26), Position=UDim2.new(0,134,1,-34),
        Text="❌ Hủy", BackgroundColor3=C.RED, TextColor3=C.WHITE,
        Font=Enum.Font.GothamBold, TextSize=10, BorderSizePixel=0, ZIndex=31,
    }, editorFrame); Corner(cancelEditBtn, UDim.new(0,5))
    ...
    saveFeatureBtn.Activated:Connect(function()
        local c = codeContent
        if #c == 0 then fStatus.Text = "⚠️ Không có code!"; return end
        local n = name; local bn = n; local cnt = 1
        while true do
            local ex = false
    ...
    -- B: cleanup sandbox khi đóng tab
...
-- ==================== TAB TẠO TÍNH NĂNG (giữ nguyên A) ====================
local createFeatureTab = AddTab("Tạo Tính Năng", "➕", 3)

local cy = 8
Label(createFeatureTab, "➕ Tạo Tab Tính Năng Tích Hợp", cy); cy += 16
Label(createFeatureTab, "Dán NGUYÊN một script hoàn chỉnh HOẶC link raw.", cy); cy += 14
Label(createFeatureTab, "Script sẽ chạy trong tab, GUI sẽ được nhúng vào menu này.", cy); cy += 18
Label(createFeatureTab, "🏷️ Tên Tính Năng:", cy); cy += 14

local featureNameIn = New("TextBox", {
    Size=UDim2.new(1,-16,0,26), Position=UDim2.new(0,8,0,cy), Text="",
    PlaceholderText="VD: Auto Farm, Fly, Speed...", PlaceholderColor3=Color3.fromRGB(160,160,160),
    BackgroundColor3=C.WHITE, TextColor3=Color3.fromRGB(20,20,20),
    Font=Enum.Font.GothamMedium, TextSize=12, BorderSizePixel=0, ClearTextOnFocus=false,
    Active=true, Selectable=true, ZIndex=10, TextXAlignment=Enum.TextXAlignment.Left,
}, createFeatureTab)
Corner(featureNameIn, UDim.new(0,5)); Stroke(featureNameIn, Color3.fromRGB(100,120,200), 1.5)
New("UIPadding", {PaddingLeft=UDim.new(0,6)}, featureNameIn)
cy += 32

Label(createFeatureTab, "🎨 Icon (1 ký tự, tùy chọn):", cy); cy += 14
...
Corner(featureIconIn, UDim.new(0,5)); Stroke(featureIconIn, Color3.fromRGB(180,180,200), 1.2)
cy += 32

Label(createFeatureTab, "📜 Dán Script Hoàn Chỉnh HOẶC link raw:", cy); cy += 14
...
Corner(featureCodeIn, UDim.new(0,5)); Stroke(featureCodeIn, Color3.fromRGB(100,120,200), 1.5)
...
Label(createFeatureTab, "━━━━━━━━━━━━━━━━━━━━━━", cy); cy += 16
Label(createFeatureTab, "📋 Danh Sách Tab Tính Năng Đã Tạo:", cy); cy += 16
...
local function RebuildFeatureList()
    ...
    if #featureTabs == 0 then
        New("TextLabel", {
            Size=UDim2.new(1,0,0,30), Text="📭 Chưa có tab tính năng nào.",
            BackgroundTransparency=1, TextColor3=C.GRAY, Font=Enum.Font.GothamMedium,
    ...
    for i, ft in ipairs(featureTabs) do
        local row = New("Frame", {
            Size=UDim2.new(1,0,0,32), BackgroundColor3=C.WHITE,
        ...
        local goBtn = New("TextButton", {
            ...
            Text="➡ Mở", BackgroundColor3=C.BLUE, TextColor3=C.WHITE,
            Font=Enum.Font.GothamBold, TextSize=9, BorderSizePixel=0, ZIndex=8,
        }, row); Corner(goBtn, UDim.new(0,4))
        goBtn.Activated:Connect(function() SwitchTab(ft.tabIdx) end)

        local delBtn = New("TextButton", {
            Size=UDim2.new(0,24,0,22), Position=UDim2.new(1,-26,0,5),
            Text="🗑", BackgroundColor3=C.RED, TextColor3=C.WHITE,
            Font=Enum.Font.GothamBold, TextSize=10, BorderSizePixel=0, ZIndex=8,
        }, row); Corner(delBtn, UDim.new(0,4))
        delBtn.Activated:Connect(function()
            local idx = nil
            for j, t in ipairs(tabs) do if t == ft.btn then idx = j; break end end
            if idx then
                if activeTab == ft.frame then SwitchTab(1) end
                -- B: cleanup sandbox trước khi destroy frame
                SandboxCleanup()
                ft.btn:Destroy()
                ft.frame:Destroy()
...
createTabBtn.Activated:Connect(function()
    ...
    if #n == 0 then createStatus.Text = "⚠️ Vui lòng nhập tên tính năng!"; return end
    if #c == 0 then createStatus.Text = "⚠️ Vui lòng dán script!"; return end
    for _, ft in ipairs(featureTabs) do
        if ft.name == n then createStatus.Text = "⚠️ Tên tính năng đã tồn tại!"; return end
    end
...
clearFormBtn.Activated:Connect(function()
    featureNameIn.Text = ""; featureIconIn.Text = "⚙️"; featureCodeIn.Text = ""
    createStatus.Text = "🧹 Đã xóa form"
end)

RebuildFeatureList()

-- ==================== TOGGLE + DRAG (giữ nguyên A) ====================
local function ToggleMainFrame()
    main.Visible = not main.Visible
    togBtn.Text = main.Visible and "✕" or "🍌"
end

closeBtn.Activated:Connect(function() main.Visible = false; togBtn.Text = "🍌" end)

dragLockBtn.Activated:Connect(function()
    S.dragMenu = not S.dragMenu
    if S.dragMenu then
        dragLockBtn.Text = "🔓"; dragLockBtn.TextColor3 = C.BLUE
    else
        dragLockBtn.Text = "🔒"; dragLockBtn.TextColor3 = Color3.fromRGB(120,120,130)
    end
end)
...
local togDragStart, togStartPos, togMoved = nil, nil, false

trackConn(togBtn.InputBegan:Connect(function(i)
    if i.UserInputType == Enum.UserInputType.MouseButton1 or i.UserInputType == Enum.UserInputType.Touch then
        if S.dragMenu then
            togDragging = true; togDragStart = i.Position
            togStartPos = togBtn.Position; togMoved = false
        end
    end
end))
...
trackConn(UserInputService.InputEnded:Connect(function(i)
    if i.UserInputType == Enum.UserInputType.MouseButton1 or i.UserInputType == Enum.UserInputType.Touch then
        if togDragging then
            togDragging = false
            if not togMoved then ToggleMainFrame() end
        end
    end
end))

togBtn.Activated:Connect(function()
    if not S.dragMenu then ToggleMainFrame() end
end)

trackConn(UserInputService.InputBegan:Connect(function(i, gp)
    if not gp and i.KeyCode == Enum.KeyCode.RightControl then ToggleMainFrame() end
end))
...
-- ============================================================
-- CÀI HOOK SANDBOX (sau khi mọi thứ UI đã tồn tại)
-- ============================================================
InstallScreenGuiHook()
InstallRunServiceHook()
InstallThreadHook()

print("✅ Banana Cat Hub FUSION: giữ nguyên bản A + Sandbox Layer B — sẵn sàng!")